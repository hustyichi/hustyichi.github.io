...
自 2025 年下半年以来，AI 编程技术迎来了爆发式发展。然而，尽管大模型在代码生成能力上突飞猛进，它们在实际应用中依然呈现出一种现象：在小型项目或从零开始的新项目中表现优异，但在面对历史包袱重、逻辑复杂的大型代码库时，往往显得有些捉襟见肘。

这一困境的核心原因在于：**大型项目通常存在庞大而复杂的上下文依赖**。实际开发中需要考虑的代码约束、全局状态和历史设计非常多，而大模型由于上下文窗口的限制或检索能力的不足，很难做到面面俱到。这就像是一个极其聪明但完全不了解你团队业务背景的“空降”技术专家，很难直接给出完美契合当前架构的代码。

在过去的解决方案中，诸如 Claude Code 等开发辅助工具主要依赖传统的 CLI 工具（如 `grep`）来帮助模型快速检索相关信息。这种基于文本精确匹配的方案，在面临语义层面的查询时往往力不从心。虽然现代大模型通过增强意图理解，在一定程度上优化了查询条件的构建，但依然无法突破精确匹配的固有局限。
...
- **中间的文本处理层（代码分片）**：这是整个方案的关键所在。为了保证代码片段的语义完整性，项目优先采用基于 AST（抽象语法树）的切分方案；当代码块过大时，再执行必要的二次切分。

## 检索方案

Claude-context 采用了**向量检索 + BM25 稀疏检索**的混合检索方案。具体实现在 `packages/core/src/context.ts` 的 `semanticSearch()` 方法中。

得益于底层 Milvus 向量库的强大支撑，这部分的代码设计极为简洁。系统只需将用户的查询意图转换为 Milvus 的混合检索请求，并在最后使用 **RRF（Reciprocal Rank Fusion，倒数排名融合）** 算法对检索结果进行重排。简化后的核心逻辑如下：
...
Claude-context 默认采用了**基于 AST（抽象语法树）的分片方案**。其核心思想是：利用 `tree-sitter` 将源码解析为语法树，随后按语言特性中的“逻辑单元”（如函数、类、方法、接口等）进行切分，从而最大限度地保留代码的结构完整性。具体实现位于 `packages/core/src/splitter/ast-splitter.ts` 中。

该方案的执行流程可以总结为：**优先尝试提取 AST 中的逻辑单元作为 Chunk；若语言不支持或解析失败，则退化为基础的字符分片。**
...
3. **加载节点白名单**：针对每种语言，项目预定义了适合作为独立 Chunk 的节点类型。例如，TypeScript 的白名单中包含了 `function_declaration`、`class_declaration`、`interface_declaration` 等。
...
在实际开发中，代码库是频繁变动的。每次代码修改如果都触发全库的重新向量化，显然是不切实际的。因此，系统必须具备高效的差异发现与增量更新机制。

基于文件内容计算 SHA-256 哈希并在本地保存快照是很容易想到的做法，但持续对大量文件进行逐个比对依然较慢。为此，项目引入了类似 [Merkle DAG](https://docs.ipfs.tech/concepts/merkle-dag/) 的思想来进行快速的全局差异判定。

虽然名字听起来高深，但它的核心思想其实非常朴素：通过不断地哈希聚合，将整个代码库的信息压缩成一个固定长度的指纹（Root Hash）。这样可以方便进行快速的差异确认。在常规情况下会针对每个目录构建目录级别的 Merkle DAG，可以逐个目录确认是否存在变更。

![merkle_tree](/img/in-post/claude-context/merkle_tree.png)

Claude-context 的设计更为精简：它没有使用严格的多层级目录树结构，而是一个挂载所有文件节点的 Root 节点，这样可以快速确认整个代码库是否有变更。
