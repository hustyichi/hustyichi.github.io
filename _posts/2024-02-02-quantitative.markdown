...
## 背景介绍
掌握技能最快的方法就是用技能创造产品，前一阵子对 GPT 辅助编程与项目生成做过比较多的调研，但是一直处于试水阶段，没有发挥出 GPT 辅助编程的能力。

本次从新领域入手，借助 GPT 的力量，从 0 手撸一个量化交易回测平台，从这个过程看看 GPT 辅助编程到底有多强，能多大程度帮助程序员，甚至替代程序员。

## 效果展示
最终花费 2 周的休息时间，耗时 116 个番茄钟，从 0 撸出一个量化交易回测的服务。当前支持了 Windows，Linux，Mac 平台，本地一键启动，支持一秒化身量化交易回测服务器。效果如下：

接入了实时和离线的数据源，实时数据源来自于 yahoo finance，内置了 KDJ, MACD, 突破，动量与海龟策略，支持策略参数动态调节：

![operations](/img/in-post/quantitative/operations.png)

回测的效果支持了交易回测 K 线图，账户价值变动与年化收益率曲线，最重要的还支持了详细交易记录的展示与导出：
...
对项目模块做了很好的分拆，其中 `data_manager.py` 用于数据管理，`strategy.py` 用于策略管理，`visualization.py` 用于可视化，甚至包含相关依赖文件和启动脚本。所以一切已经就绪？
...
后续又补充了必要的技术选型的细节之后让 GPT Engineer 进行生成，依旧表现不佳，只能生成偏框架流程的代码，看起来 GPT Engineer 靠单纯组织 prompt 希望实现一个完备的项目只能适用于极小的项目需求，复杂需求下不太实用。


#### 技术选型

依赖 GPT 直接生成项目不可行，接下来就得考虑建立在哪些库的基础上实现完整的项目，合适的技术选型可以帮助尽可能减少开发工作量，同时保证项目的质量。
...
Dash 可以快速搭建可视化 Web 服务，开源版本的包含的组件的丰富度和美观度都相对较好。Backtrader 框架提供的能力完备，而且可维护性强，关键是相对其他库上手成本低。

#### 模块级开发实践

之前常规的项目开发时，一般都会先搭建整体的服务框架，然后在服务框架的基础上填充功能。本次在人 + GPT 协作开发全新的服务过程中，实际尝试下来采取积木组装的方式可能效果更好。

根据前面 GPT-Engineer 的实践以及之前更强大的全自动的 Auto-GPT 使用的效果来看，目前阶段 GPT 很难支撑全自动复杂框架的开发，但是范围边界明确的小模块的开发是可行的。为了充分利用 GPT 现有的能力，因此人工可以对完整的需求进行拆分，然后用更小的模块进行代码生成，之后再进行积木式的组装。

在实际的量化交易回测的服务中，我将原始的需求拆分为如下所示的模块：
...
虽然每个模块拆分后可以分块按照顺序推进，但是考虑到新领域的不熟悉，可能存在很多不确定性，因此采取敏捷管理的思路，先生成部分小模块，组装出一个小积木。先实现离线的数据管理模块，然后实现 KDJ 策略，再完成回测效果的可视化，组装出一个最小的 MVP。

实际生成单个模块的体验还是比较友好的，在框架和需求明确的情况下，GPT 们都会给出对应的代码实现：

![kdj](/img/in-post/quantitative/kdj.png)

看起来是不是很轻松，GPT 甚至连测试代码都生成好了，看起来程序员需要做的就复制粘贴，然后组装起来就好。但是实际中还是会碰到一些问题，主要的问题来自于人往往有自知之明，但是 GPT 很显然没有。GPT 有时候会给出看起来正确，但是实际存在的问题的实现。

有时候的是因为第三方库的接口发生了变更，而 GPT 实现的依旧是老版本的代码，有时候使用不存在的库或其他的问题，但是 GPT 带来的好处依旧是有的，因为测试代码一般都是一起生成的，有些不靠谱的实现很快就能排除。
...
比如实现 KDJ 策略不正确，从原理上了解到是基于 K,D,J 三个指标，然后基于指标确定买入卖出机制，可以让 GPT 先生成正确的三个指标，然后确定指标的实现的正确性之后，再基于指标实现交易的金叉买入，死叉卖出的机制。

![kdj_index](/img/in-post/quantitative/kdj_index.png)

在完成各个模块的开发之后再进行必要的组合，甚至可以将组合的过程也看成一个特定的模块，实现类似的方式进行拼装开发。

即便如此，实际中还是碰到一些 GPT 无法解决，需要去研究官方文档去了解基础框架的实现原理，考虑到项目中使用的基础框架确定后就会产生大量的依赖，而 GPT 对基础框架的了解存在时效性或知识不足的问题，因此一个可能的解决方案就是基于 [RAG](https://arxiv.org/abs/2005.11401), 通过外接向量化的官方文档知识库解决 GPT 存在的时效性和了解不足的问题，目前已经一些基于 [langchain](https://www.langchain.com/) 的解决方案，比如 [langchain-chatchat](https://github.com/chatchat-space/Langchain-Chatchat)，这个后续会抽空做一些调研，希望能进一步提升开发效率。

## 总结
文章本来是希望介绍量化交易回测工具的从 0 手撸的实现原理，但是实际想想，授人予鱼不如授人予渔，单个量化交易回测工具作用有限，但是用好了 GPT 工具，只需要花上一丢丢时间，你也能创造一个。
