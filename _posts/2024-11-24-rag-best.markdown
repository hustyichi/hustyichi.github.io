...
从之前的 [QAnything](https://zhuanlan.zhihu.com/p/697031773) 的调研就发现，Rerank 对 RAG 检索准确性的提升十分明显，在实践中也很容易感受到在这种差异，因此在最近的 RAG 方案中 Rerank 基本成为标配方案了。

当前的问题就是如何选择 Rerank 模型，在论文中进行了不同类型的 Rerank 模型的比较，具体的结果如下所示：

![rerank](/img/in-post/rag-best/rerank.png)

论文从平衡性能和效果的角度推荐使用 monoT5，从我实践的情况来看，[bge-reranker-v2-m3](https://huggingface.co/BAAI/bge-reranker-v2-m3) 是一个在生产环境相对不错的选择，如果追求更好的效果，我实际建议的方案与之前 [AIOps RAG 比赛](https://zhuanlan.zhihu.com/p/3758214821) 中的建议一致，建议使用使用基于 LLM 的 Rerank 模型 [bge-reranker-v2-minicpm-layerwise](https://huggingface.co/BAAI/bge-reranker-v2-minicpm-layerwise)。
...
- 基于文本长度限制过滤，因为最终大模型可以支持的输入上下文是有限，因此只能选择 topN 的相似文档；
...
在论文中的结果来看，Recomp 的效果相对更好，在 [AIOps RAG 比赛](https://zhuanlan.zhihu.com/p/3758214821) 中实现了一个基于 BM25 的检索压缩方案，我在实践中进行了测试，效果不是特别理想。最终参考 AIOps RAG 比赛中的方案，将 BM25 评分替换为 Rerank 评分，可以取得了更好的效果，当然成本也会更高。
...
从论文的结果来看，对效果的影响相对有限，我猜测可能是因为检索实际匹配的文档数量有限，`Lost in middle` 效应的影响没那么明显。从目前来看，如果实现简单，可以考虑排列在两侧的方案。


## 总结

本文是基于复旦大学的论文，AutoRAG 以及个人的实践经验模块化梳理了当前 RAG 的最佳实践方案，按照这个模块化的方案可以相对低成本的组装出一个相对不错的 RAG 方案，虽然实际中可能依旧需要根据具体场景进行特定调整，但是已经是一个相对良好的基础方案了，而且模块化方案后续的迭代升级也相对方便。
