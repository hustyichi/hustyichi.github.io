...
## 背景介绍
在 [前一篇文章](https://zhuanlan.zhihu.com/p/701979814) 中介绍过智谱组织的一个金融大模型 RAG 比赛 [FinGLM](https://github.com/MetaGLM/FinGLM)，上一篇文章主要介绍的是获得第三名的项目，这篇文章主要介绍下比赛最终获得冠军的馒头科技的技术方案。

不了解项目背景信息的可以先查看 [来自工业界的知识库 RAG 服务(三)，FinGLM 竞赛获奖项目详解](https://zhuanlan.zhihu.com/p/701979814)
...
3. 关键词抽取，就是流程图中的意图识别模块，因为部分问题的处理需要根据关键词提取，因此需要具备准确提取关键词的能力；

上面的几个核心功能都是基于微调 ChatGLM-6B 模型来实现的。

#### 微调模型预处理

上面提到的几个预处理的功能都是通过微调大模型实现的，处理模块都是大同小异，以问题分类为例查看其中的预处理流程，实际的调用如下所示：
...
可以看到基于微调模型的方案进行问题分类，实现过程相对简单，只需要合适的 prompt 即可。

关键词抽取和 SQL 生成的流程也是类似的，在 SQL 生成中还额外在 prompt 使用 Few Shot 机制优化了效果，但是整体差异不大。


#### 文本召回与回答

在经过前面的预处理环节后，接下来就可以基于根据问题分类的类型选择不同的数据源进行召回，并根据召回的信息生成最终的结果。
...
需要公式类的问题数据召回和回答的流程如下所示：

![type2](/img/in-post/finglm2/type2.png)

因为 ChatGLM-6B 实际进行数学计算时表现不佳，因此需要进行外部计算，根据匹配的关键词确定实际需要采取的计算公式，之后需要获取原始的数据源。使用原始的数据源进行 Python 计算，之后将得到的结果提供给大模型输出最终的结果。
...
增长率是一个特殊的公式构造，因为需要获取前一年的数据，因此需要特殊构造。除此以外，其他就是根据实际的公式确定计算所需的字段，之后就可以生成对应的问题。

实际计算的过程就是通过调用上面生成的公式链获取所需的原始数据，之后提取所需的字段的数值，调用 Python 提供的 `eval()` 进行计算得到最终结果。实现过程如下所示：
...
这种类型的问题比较类似常规的 RAG 系统中面临的问题，与常规的 RAG 系统基于向量检索不同，这边使用的 BM25 检索，直接使用关键字进行检索。
...
## 总结
本文主要整理了 FinGLM 获得冠军的团队提供的方案，从处理流程来看，亮点不是特别突出，最终表现良好与其精细的微调的模型应该有不少的关系。

结合目前梳理的几个获奖项目，存在如下一些值得注意的经验：

1. 特定场景的大模型微调可以大幅提升效果，可以在必要的场景下进行尝试；
