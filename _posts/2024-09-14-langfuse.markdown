...
为了进行批量自动化测试，需要获取数据集中所有的测试项，依次提交给 Dify 服务，并将 Dify 返回的消息 id 与 Langfuse 中的 trace id 进行关联，类似如下所示：
...
常规的自动化评估可以自由选择，只需要计算评估得分后调用 Langfuse 中的 `score()` 方法，将得分传入 Langfuse 即可。常规 RAG 的评估框架之前的文章中 [从开发到部署，搭建离线私有大模型知识库](https://zhuanlan.zhihu.com/p/689947142), 可以选择 ragas, trulens 等。
...
个人选择 ragas 作为评估方法，评估完成后可以测试列表如下所示：

![after_test](/img/in-post/langfuse/after_test.png)

## 总结

本文是对 Dify + Langfuse 的初步实践，从个人使用体验来看，由于 Dify 官方提供了完整的支持，因此 Langfuse 的接入变得相对简单，可以快速跟踪现有大模型应用的执行链路。

但是目前 Langfuse 还处于快速迭代中，数据集的构建以及自动化测评的集成还不是很完善，需要自己做必要的代码开发，才能真正满足自动化测试需求。但是整体而言，在 Dify 应用中，Langfuse 还是一个不错的选择，推进大家尝试一下。
